...
from closed_loop.xyz_normalization import denormalize_xyz_torch
...
class SelfAttnBlock(nn.Module):
    ...
    def forward(
        ...
        x, _ = self.self_attn(
            h_self, h_self, h_self, key_padding_mask=key_padding_mask, need_weights=False
        )
        h = h + x
...
class ActionTrajectoryModel(nn.Module):
    ...
    def _ref_token(self, clip_emb: torch.Tensor, xyz_norm: torch.Tensor) -> torch.Tensor:
    ...
    def build_sequence(
        ...
        *,
        ...
        for i in range(bsz):
            ...
            if hm and hd:
                hi = torch.cat(
                    [h_instr[i], h_tool[i], h_material[i], h_destination[i], h_table[i], h_noise[i]],
                    dim=0,
                )
        ...
        return torch.stack(h_list, dim=0), torch.stack(mask_list, dim=0)
    ...
    def postprocess_waypoints(
        ...
        wp = pred_norm.view(
            -1, ActionTrajectoryModel.NUM_WAYPOINTS, ActionTrajectoryModel.WAYPOINT_DIM
        )
