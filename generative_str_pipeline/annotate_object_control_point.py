...
Click four corners on the bristle face (front L/R, back L/R) on the mesh in the browser,
regularize to a rectangle, compute the contact frame, verify arrows, and save JSON.

Usage (from repo root, with generative_str_pipeline venv):

    python -m generative_str_pipeline.annotate_object_control_point \\
...
def default_output_dir() -> Path:
    return Path(__file__).resolve().parent / "assets" / "object_control_points"
...
class ObjectControlPointAnnotator:
    def __init__(self, args: AnnotateArgs) -> None:
        self.args = args
        obj_path = args.obj_path or default_obj_path(args.object_name)
        if not obj_path.is_file():
            raise FileNotFoundError(f"Mesh not found: {obj_path}")

        self.obj_path = obj_path.resolve()
        self.mesh: trimesh.Trimesh = trimesh.load(str(self.obj_path), force="mesh")
        if not isinstance(self.mesh, trimesh.Trimesh):
            raise TypeError(f"Expected Trimesh, got {type(self.mesh)}")
    ...
    def _setup_gui(self) -> None:
        ...
        self.server.gui.add_markdown(f"**Object:** `{self.args.object_name}`")
    ...
    def _save(self) -> None:
        ...
        out_path = self.output_dir / f"{self.args.object_name}.json"
        frame = self.computed
