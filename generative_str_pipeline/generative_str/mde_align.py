...
def align_run_directory(
    ...
    *,
    ...
    Requires ``mde_raw_dir/frame_0000.png`` (MDE on the first RGB) to fit scale/shift
    against ``capture_dir/depth/frame_0000.png``.

    Depth PNGs: if max > 100, values treated as **millimeters**; else **meters**.
    Output depths are uint16 PNG in millimeters (ZED-style).

    Copies RGB from ``capture_dir/rgb/`` and ``<capture_dir.parent>/video_gen/rgb/`` if present.
    Writes ``depth/frame_0000.png`` from **camera GT**; other frames as ``s * MDE + b`` (meters).
    """
    ...
    if not mde0_path.is_file():
        raise FileNotFoundError(
            f"{mde0_path} missing — run MDE on frame_0000 for alignment"
        )
