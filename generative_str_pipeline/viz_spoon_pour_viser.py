"""Interactive viser viz of the spoon scoop-and-pour trajectory with a real mesh.

Rolls out dataset_0013's analytic closed-loop spoon serve motion, then animates
the actual ``spoon_spatula`` mesh via its annotated control frame.

Usage:
    python -m generative_str_pipeline.viz_spoon_pour_viser --seed 3 --port 8080
"""
...
def build_pour_frames(
    *, seed: int, T_oc: np.ndarray, chunk: int
) -> Tuple[List[PourFrame], dict]:
    ...
    for dp in datapoints:
        ...
        mat_quat = np.asarray(dp["material_quat_executed_world"], dtype=np.float64).reshape(-1, 4)
        ...
        for i in range(n_take):
            ...
            frames.append(PourFrame(txyz, tquat, contact, mat_xyz[i], mat_quat[i], gen))
...
class SpoonPourViz:
    def __init__(self, *, control_frame: str, seed: int, port: int, chunk: int, fps: float):
        ...
        self.frames, self.meta = build_pour_frames(seed=self.seed, T_oc=self.T_oc, chunk=self.chunk)
    ...
    def _setup_gui(self) -> None:
        self.server.gui.add_markdown("# Spoon scoop-and-pour — closed-loop plan")
        self.server.gui.add_markdown(
            "Spoon comes in from the front, scoops the material, lifts above the "
            "goal, and rolls sideways to pour."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Viser spoon scoop-and-pour closed-loop viz.")
    parser.add_argument("--control-frame", default="spoon_spatula")
